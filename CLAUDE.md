# Claude Code Project Instructions — quick-xml-kotlin

## Project Overview

This is **quick-xml-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`quick-xml`](https://crates.io/crates/quick-xml).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/quick-xml/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/quick-xml>
