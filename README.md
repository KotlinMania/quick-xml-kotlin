...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`quick-xml`](https://crates.io/crates/quick-xml).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `quick-xml` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `quick-xml` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
